...
- **Stack:** React + Vite + Tailwind (web, hosted on Vercel), Firebase Auth / Firestore / Storage, Node.js + Express server (write logic, pricing, payments, Geoapify proxy, emails).
...
label (Home, Work), formattedAddress, lat, lng, phone, landmark, notes, isDefault, source (vhitemap | geoapify | manual).
...
- GET /geocode/search and GET /geocode/reverse: Geoapify proxy, rate-limited
...
2. Use two API keys in that project: a private one for the Node server (kept in `.env`, never in the browser) and a separate one for VhiteMap, which runs in the browser. Restrict the VhiteMap key to VhiteMap's domain (and localhost for development) with the allowed origins/referrers setting.
3. Watch daily usage in the Geoapify dashboard.
