...
function targetTime(order) {
  const value =
    order.requestedByAt?.toDate?.() ||
    (order.requestedByAt ? new Date(order.requestedByAt) : null);
...
function deliveryTarget(order, now = Date.now()) {
  const target = targetTime(order);

  if (!target || ["delivered", "cancelled"].includes(order.status)) {
    return null;
  }

  const difference = target - now;
  const absoluteMinutes = Math.floor(Math.abs(difference) / 60000);

  if (difference < 0) {
    return {
      state: "late",
      label: absoluteMinutes === 0
        ? "Late now"
        : `${absoluteMinutes} min late`,
    };
  }

  return {
    state: difference <= 15 * 60000 ? "soon" : "on-time",
    label:
      absoluteMinutes === 0
        ? "Due now"
        : `${absoluteMinutes} min remaining`,
  };
}
...
function urgency(order) {
  ...
  if (target?.state === "late") return "late";

  if (target?.state === "soon") return "warning";
...
function urgencyLabel(value) {
  ...
  if (value === "late") return "Running late";
...
export default function StaffOrders({
  ...
  return (
    <div className="v2-orders-workspace">
        ...
        <div className="v2-order-list">
          {filteredOrders.length === 0 ? (
            <div className="v2-empty-orders">
              ...
              <p>
                Try changing the search, status or
                urgency filter.
              </p>
            </div>
          ) : (
            filteredOrders.map((order) => (
              <StaffOrderRow
                ...
                onReject={() =>
                  openReason(order, "reject")
                }
                onCancel={() =>
                  openReason(order, "cancel")
                }
                onClaim={() =>
                  run(order, () =>
                    claimOrder(order.id)
                ...
                onProblem={() =>
                  openReason(order, "problem")
                }
                onRefundDone={() =>
                  run(order, () =>
                    markRefundDone(order.id)
                  )
                }
              />
            ))
      ...
      {reasonFor && (
        <div
          className="v2-reason-backdrop"
          onClick={() => setReasonFor(null)}
        >
          <div
            className="v2-reason-modal"
            onClick={(e) =>
              e.stopPropagation()
            }
          >
            <span>
              {reasonFor.kind === "problem"
                ? "FLAG A PROBLEM"
            ...
            <p>
              {reasonFor.kind === "problem"
                ? "Describe what the kitchen or delivery team needs attention on."
...
function StaffOrderRow({
  ...
  return (
    <article
      className={`v2-order-row urgency-${orderUrgency}`}
    >
      ...
      <div className="v2-order-status">
        <span
          className={`v2-status status-${order.status}`}
        >
        ...
        {target && (
          <span
            className={`v2-target-state target-${target.state}`}
          >
            <Clock3 size={12} />
            {target.label}
          </span>
